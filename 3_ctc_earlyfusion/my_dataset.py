...
class SequenceDataset(Dataset):
    ...
    def __init__(self,
        ...
        # 特徴量の次元数
        # フレーム数×次元数×8chの値になっているが，
        # ８で割って1ch分の大きさに変換している
        self.feat_dim = \
    ...
    def __getitem__(self, idx):
        ...
        feat_ori = np.load(self.feat_list[idx])

        for i in range(ch):
            # ich目のデータを抽出
            feat_0 = feat_ori[:,:,i]
            # フレーム数 x 次元数の配列に変形
            # saveとloadでデータを取り出したので，変形する必要がない
#            feat = feat.reshape(-1, self.feat_dim)
            # 平均と標準偏差を使って正規化(標準化)を行う
            feat_0 = (feat_0 - self.feat_mean[:,i]) / self.feat_std[:,i]

            # splicing: 前後 n フレームの特徴量を結合する
            feat_copy= feat_0.copy()
            for n in range(-self.splice, self.splice+1):
                # 元々の特徴量を n フレームずらす
                tmp = np.roll(feat_copy, n, axis=0)
                if n < 0:
                    # 前にずらした場合は
                    # 終端nフレームを0にする
                    tmp[n:] = 0
                ...
                # ずらした特徴量を次元方向に
                # 結合する
                feat_0 = np.hstack([feat_0,tmp])

            # 特徴量データのフレーム数を最大フレーム数に
            # 合わせるため，0で埋める
            pad_len = self.max_feat_len - feat_len
            feat_0 = np.pad(feat_0,
                          [(0, pad_len), (0, 0)],
                          mode='constant',
                          constant_values=0)

            # 8chを結合する　=> vstackによって8×75になっちゃってる
            #                　ベクトルにするならhstackにする
            if i < 1:
                feat = feat_0
            else:
                feat = np.hstack([feat, feat_0])
