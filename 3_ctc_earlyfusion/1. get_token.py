...
#
# ラベル中のトークンを，
# ニューラルネットワークで扱うため，文字から番号へ変換する
# 文字－番号の対応を記したトークンリストも作成
#
...
if __name__ == "__main__":
    ...
    # ラベルファイルの存在するフォルダ
    label_dir_train = '../data/label/train'
    ...
    for uid, unit in enumerate(unit_set):
        ...
        #
        # 学習データのラベルファイル
        label_train = os.path.join(label_dir_train, unit)
        # トークンリストを空リストで定義
        token_list = []
        # 訓練データのラベルに存在するトークンを
        # token_listに登録する
