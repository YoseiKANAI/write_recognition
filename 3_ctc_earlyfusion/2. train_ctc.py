...
if __name__ == "__main__":
    ...
    feat_dir_train = '../2_compute_features/fft/train'
    # 開発データの特徴量(Feats.scp)が存在するディレクトリ
    feat_dir_dev = '../2_compute_features/fft/dev'
    ...
    # meanとstd用の空のarray配列を作成
    feat_mean = np.empty((frame_size, ch))
    feat_std  = np.empty((frame_size, ch))

    # 特徴量の平均/標準偏差ファイルを読み込む
    # feat_mean,stdはそれぞれの特徴量が格納されたarray配列
    # [8, フレーム数]となっている
    for i in range(ch):
        with open(mean_std_file, mode='r') as f:
            ...
            # 3行目が標準偏差ベクトル(std)
            mean_line = lines[1+(i*4)]
            std_line = lines[3+(i*4)]
            # スペース区切りのリストに変換
            feat_mean_0 = mean_line.split()
            feat_std_0 = std_line.split()
            # numpy arrayに変換
            feat_mean_0 = np.array(feat_mean_0,
                                    dtype=np.float32)
            feat_std_0 = np.array(feat_std_0,
                                   dtype=np.float32)

            # 20×8の配列
            feat_mean[:, i] = feat_mean_0
            feat_std[:, i] = feat_std_0
    ...
    # 次元数128の場合
#    feat_dim = 128
    # ベクトルの場合追加(early fusion)
    feat_dim = feat_mean.size
    ...
    # 8chのデータセットをベクトルとしてリストに格納
    # i+1chの訓練データのデータセットを作成する
    train_dataset = SequenceDataset(feat_scp_train,
                                      label_train,
                                      feat_mean,
                                      feat_std)
    # i+1chの開発データのデータセットを作成する
    ...
    # 呼び出すときは，辞書のキーからリスト全体を呼び出した後，
    #           リストからほしいchのデータを呼び出せばよい
    ...
    for epoch in range(max_num_epoch):
        ...
        for phase in ['train', 'validation']:
            ...
            # 各フェーズのDataLoaderから1ミニバッチずつ取り出して処理する．
            ...
            for (features, labels, feat_lens,
                ...
                # modelはMyCTCModelクラスの変数
                # エンコーダに値をそのまま入力し，enc_outとenc_lenを得る　encorder.py
                # その後enc_outを出力層（Projection層）に入力する　my_nodel.py
